...
struct assoofs_inode_info *assoofs_get_inode_info(struct super_block *sb, uint64_t inode_no){
        ...
        printk(KERN_INFO "assofs_get_inode_info\n");
...
ssize_t assoofs_read(struct file * filp, char __user * buf, size_t len, loff_t * ppos) {
    ...
    cpUser=copy_to_user(buf, buffer, nbytes);
...
ssize_t assoofs_write(struct file * filp, const char __user * buf, size_t len, loff_t * ppos) {
    ...
    cpUser=copy_from_user(buffer, buf, len);
    
   if(cpUser!=0) printk(KERN_ERR "No se ha podido leer el número de bits\n");
...
struct assoofs_inode_info *assoofs_search_inode_info(struct super_block *sb, struct assoofs_inode_info *start, struct assoofs_inode_info *search){
    ...
    printk(KERN_INFO "assofs_search_indode_info\n");
    ...
    printk(KERN_INFO "contador: %llu\n", count);
    printk(KERN_INFO "inodes_count %llu\n",((struct assoofs_super_block_info *)sb->s_fs_info)->inodes_count);
...
void assoofs_save_sb_info(struct super_block *vsb){
        ...
        printk(KERN_INFO "ASSOFS_SAVE_SB_INFO ENTRANCE");
...
int assoofs_sb_get_a_freeblock(struct super_block *sb, uint64_t *block){
   ...
   for (i = 2; i < ASSOOFS_MAX_FILESYSTEM_OBJECTS_SUPPORTED; i++)
    ...
    if(*block>ASSOOFS_MAX_FILESYSTEM_OBJECTS_SUPPORTED){
     ...
     printk(KERN_INFO "El bloque libre es %d", i);
...
static struct inode *assoofs_get_inode(struct super_block *sb, int ino){
    ...
    inode->i_private = inode_info; // Información persistente del inodo
...
static void __exit assoofs_exit(void) {
 ...
 if(ret!=0) printk(KERN_ERR "Error during register");
}
